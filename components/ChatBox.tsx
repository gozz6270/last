...
export default function ChatBox({
  ...
  const createSystemPrompt = (data: ChatBoxProps["questionData"]) => {
    ...
    const prompt =
      ...
      "   - **기본값(디폴트): 3~4단계** (totalSteps = 3 또는 4)\n" +
      "   - 매우 간단한 문제만 1~2단계로 줄인다 (단순 계산, 한 번의 연산으로 끝나는 문제)\n" +
      "   - 예: '$2 + 3 = ?$' 같은 직접 계산 → 1단계 OK\n" +
      "   - 예: '$2x = 6$' 같은 단순 방정식 → 2단계 OK\n" +
      "   - 예: '$2x^2 - 5x - 3 = 0$' 같은 일반적인 문제 → 3~4단계 (디폴트)\n" +
      "   - 복잡한 문제는 4~5단계까지 가능 (totalSteps = 4 또는 5)\n" +
      "   - 중요: 의미 있는 풀이 단계를 충분히 보여줘야 한다!\n\n" +
      "2) 각 단계는 실제 문제 해결에 필수적인 과정만 포함하라:\n" +
      "   - 좋은 단계: 식 변형, 인수분해, 대입, 계산, 해 구하기 등 실제 풀이 단계\n" +
      "   - 나쁜 단계: '문제 이해하기', '전략 세우기', '검증하기' 같은 불필요한 단계\n" +
      "   - 각 단계는 반드시 정답에 한 걸음 가까워지는 구체적 작업이어야 함\n\n" +
      "3) options(선택지) 구성 규칙:\n" +
      "   - 반드시 실제 계산식/구체적 값/식 변형을 담아라 (추상적 설명 금지!)\n" +
      '   - 좋은 예: ["$2x^2 - 5x - 3 = (2x + 1)(x - 3)$", "$x = 3$ 또는 $x = -\\\\frac{1}{2}$"]\n' +
